...
export function Stage({ timeline, input, sending, onInputChange, onSend }: StageProps) {
  return (
    <Panel className="flex h-full flex-col p-4 md:p-5">
      <div className="mb-4 border-b border-slate-200/70 pb-3 text-sm font-semibold text-slate-800">Conversation Stage</div>
          ...
          if (item.kind === "thought") {
            return (
              <details key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                <summary className="cursor-pointer text-xs font-semibold text-slate-600">Thought</summary>
            ...
            return (
              <div key={item.id} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                <div className="flex items-center gap-2 text-slate-700">
                  ...
                  <span className="font-semibold">{item.kind === "tool_call" ? "Tool Call" : "Tool Result"}</span>
          ...
          if (item.kind === "error") {
            return (
              <div key={item.id} className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-700">
                <div className="mb-1 flex items-center gap-2 font-semibold"><CircleAlert className="h-4 w-4" /> Error</div>
